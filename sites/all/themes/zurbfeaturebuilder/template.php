...
function _zurbfeaturebuilder_load($files) {
  $tp = drupal_get_path('theme', 'zurbfeaturebuilder');
  $file = '';
...
_zurbfeaturebuilder_load($files);

/**
 * Implements hook_html_head_alter().
 */
function zurbfeaturebuilder_html_head_alter(&$head_elements) {
 ...
 */
function zurbfeaturebuilder_breadcrumb($variables) {
...
function zurbfeaturebuilder_field($variables) {
  $output = '';
 ...
 */
function zurbfeaturebuilder_field__taxonomy_term_reference($variables) {
  $output = '';
 ...
 */
function zurbfeaturebuilder_links__topbar_main_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
  $links = menu_tree_output(menu_tree_all_data(variable_get('menu_main_links_source', 'main-menu')));
  $output = _zurbfeaturebuilder_links($links);
 ...
 * Formats links for Top Bar http://foundation.zurb.com/docs/components/top-bar.html
 */
function zurbfeaturebuilder_links__topbar_secondary_menu($variables) {
  // We need to fetch the links ourselves because we need the entire tree.
 ...
 *
 * @see zurbfeaturebuilder_links__system_main_menu()
 * @see zurbfeaturebuilder_links__system_secondary_menu()
 */
function _zurbfeaturebuilder_links($links) {
  $output = '';

  foreach (element_children($links) as $key) {
    $output .= _zurbfeaturebuilder_render_link($links[$key]);
  }
 ...
 *   A rendered list of links, with no <ul> or <ol> wrapper.
 *
 * @see _zurbfeaturebuilder_links()
 */
function _zurbfeaturebuilder_render_link($link) {
  $output = '';
  ...
  if (!empty($link['#href'])) {
    ...
    $active_classes = _zurbfeaturebuilder_in_active_trail($link['#href']);
    ...
    if (!isset($rendered_link)) {
      $rendered_link = theme('zurbfeaturebuilder_menu_link', array('link' => $link));
    }
    ...
    if (!empty($link['#below'])) {
      ...
      foreach ($link['#below'] as $sub_link) {
        if (!empty($sub_link['#href'])) {
          $sub_menu .= _zurbfeaturebuilder_render_link($sub_link);
        }
      }
 ...
 */
function _zurbfeaturebuilder_in_active_trail($link_path) {
  $classes = array();
  $active_paths = array();
 ...
 */
function theme_zurbfeaturebuilder_menu_link($variables) {
  $link = $variables['link'];
 ...
 */
function zurbfeaturebuilder_preprocess_block(&$variables) {
 ...
 */
function zurbfeaturebuilder_preprocess_field(&$variables) {
 ...
 */
function zurbfeaturebuilder_preprocess_html(&$variables) {
  global $language;
  ...
  //  $font = str_replace(' ', '+', theme_get_setting('zurbfeaturebuilder_font'));
  //  if (theme_get_setting('zurbfeaturebuilder_font')) {
 ...
 */
function zurbfeaturebuilder_preprocess_node(&$variables) {
 ...
 */
function zurbfeaturebuilder_preprocess_page(&$variables) {
  ...
  // Top bar.
  if ($variables['top_bar'] = theme_get_setting('zurbfeaturebuilder_top_bar_enable')) {
    $top_bar_classes = array();

    if (theme_get_setting('zurbfeaturebuilder_top_bar_grid')) {
      $top_bar_classes[] = 'contain-to-grid';
    }

    if (theme_get_setting('zurbfeaturebuilder_top_bar_sticky')) {
    ...
    $variables['top_bar_menu_text'] = check_plain(theme_get_setting('zurbfeaturebuilder_top_bar_menu_text'));

    $top_bar_options = array();

    if (!theme_get_setting('zurbfeaturebuilder_top_bar_custom_back_text')) {
      $top_bar_options[] = 'custom_back_text:false';
    }

    if ($back_text = check_plain(theme_get_setting('zurbfeaturebuilder_top_bar_back_text'))) {
      if ($back_text !== 'Back') {
        $top_bar_options[] = "back_text:'{$back_text}'";
      }
    }

    if (!theme_get_setting('zurbfeaturebuilder_top_bar_is_hover')) {
      $top_bar_options[] = 'is_hover:false';
    }

    if (!theme_get_setting('zurbfeaturebuilder_top_bar_scrolltop')) {
  ...
  // Messages in modal.
  $variables['zurbfeaturebuilder_messages_modal'] = theme_get_setting('zurbfeaturebuilder_messages_modal');
  ...
  if(theme_get_setting('zurbfeaturebuilder_messages_modal')) {
    drupal_add_js(drupal_get_path('theme', 'zurbfeaturebuilder') . '/js/behavior/reveal.js');
  }
}
 ...
 */
function zurbfeaturebuilder_css_alter(&$css) {
  // Remove defaults.css file.
  unset($css[drupal_get_path('module', 'system') . '/system.menus.css']);

  // Remove Drupal core CSS.
  if (theme_get_setting('zurbfeaturebuilder_disable_core_css')) {
 ...
 */
function zurbfeaturebuilder_pager($variables) {
  $tags = $variables['tags'];
  ...
  if ($pager_total[$element] > 1) {
    ...
    if (theme_get_setting('zurbfeaturebuilder_pager_center')) {
 ...
 */
function zurbfeaturebuilder_theme() {
  $return = array();

  $return['zurbfeaturebuilder_reveal'] = array(
    'variables' => array(
    ...
    'function' => 'theme_zurbfeaturebuilder_reveal',
  );

  $return['zurbfeaturebuilder_menu_link'] = array(
    'variables' => array('link' => NULL),
    'function' => 'theme_zurbfeaturebuilder_menu_link',
  );
  return $return;
}
 ...
 *
 * @see theme_zurbfeaturebuilder_reveal()
 */
function _zurbfeaturebuilder_reveal($reveal = NULL) {
 ...
 *
 * @see zurbfeaturebuilder_theme()
 * @see zurbfeaturebuilder_preprocess_region()
 * @see _zurbfeaturebuilder_reveal()
 */
function theme_zurbfeaturebuilder_reveal($variables) {
  // Generate unique IDs.
  static $counter = 0;
  ...
  _zurbfeaturebuilder_reveal($reveal);
 ...
 */
function _zurbfeaturebuilder_add_reveals() {
  $markup = '';

  // Retrieve reveal markup from static storage.
  foreach (_zurbfeaturebuilder_reveal() as $reveal) {
 ...
 */
function zurbfeaturebuilder_theme_registry_alter(&$theme_registry) {
  ...
  foreach ($entity_info as $entity => $info) {
    if (isset($entity_info[$entity]['fieldable']) && $entity_info[$entity]['fieldable']) {
      ...
      if (isset($theme_registry[$entity])) {
        $theme_registry[$entity]['preprocess functions'][] = 'zurbfeaturebuilder_entity_variables';
      }
    }
  }
  ...
  if (isset($theme_registry['file_entity'])) {
    $theme_registry['file_entity']['preprocess functions'][] = 'zurbfeaturebuilder_entity_variables';
  }
 ...
 *
 * @see zurbfeaturebuilder_theme_registry_alter()
 */
function zurbfeaturebuilder_entity_variables(&$vars) {
 ...
 */
function zurbfeaturebuilder_process_html_tag(&$vars) {
  if (theme_get_setting('zurbfeaturebuilder_html_tags')) {
    $el = &$vars['element'];
 ...
 * @return string
 */
function zurbfeaturebuilder_links__magic_button($variables) {
  ...
  if (count($variables['links']) > 1) {
    switch ($variables['type']) {
      case 'split':
        return zurbfeaturebuilder_links__split_button($variables);
        break;

      case 'dropdown':
      default:
        return zurbfeaturebuilder_links__dropdown_button($variables);
        break;
    }
  }
 ...
 */
function zurbfeaturebuilder_links__dropdown_button($variables) {
  if (empty($variables['attributes']['class'])) {
    $variables['attributes']['class'] = array();
  }
  ...
  $output = _zurbfeaturebuilder_links($variables['links']);
 ...
 */
function zurbfeaturebuilder_links__split_button($variables) {
  $links = $variables['links'];
